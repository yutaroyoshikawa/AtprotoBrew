...
export function Launcher({ installedChannels, onLogout, isLoading = false }: LauncherProps) {
	const { time, date } = useDateTime();
	const navigate = useNavigate();
	const [selectedChannel, setSelectedChannel] = useState<InstalledChannel | null>(null);
	const [startedApp, setStartedApp] = useState<string | null>(null);
	const [page, setPage] = useState(0);

	const tilesPerPage = TOTAL_TILES;
	const totalPages = Math.ceil(Math.max(1, installedChannels.length) / (tilesPerPage - 1));

	const pageChannels = installedChannels.slice(page * (tilesPerPage - 1), (page + 1) * (tilesPerPage - 1));
		...
		<div className="relative w-full h-screen overflow-hidden select-none font-sans">
			...
			{isLoading && (
				<div className="absolute inset-0 z-60 flex items-center justify-center bg-black/40 backdrop-blur-sm">
					<div className="glass-tile aero-shadow rounded-3xl px-10 py-8 flex flex-col items-center gap-4">
						...
						<p className="font-bold text-slate-800 text-lg">Loading channels...</p>
					</div>
				</div>
			)}
			...
			{startedApp && (
				<div className="absolute inset-0 z-60 flex items-center justify-center bg-black/40 backdrop-blur-sm">
					<div className="glass-tile aero-shadow rounded-3xl px-10 py-8 flex flex-col items-center gap-4">
						<div className="w-16 h-16 rounded-full border-4 border-blue-400/40 border-t-blue-600 animate-spin" />
						<p className="font-bold text-slate-800 text-lg">Starting {startedApp}...</p>
					</div>
				</div>
			)}
			...
			<div className="relative z-10 flex flex-col h-full">
				<header className="flex items-center justify-between px-6 pt-4 pb-2">
					<div className="flex items-center gap-2">
						<div
							...
							style={{
								...
								boxShadow: "0 2px 8px rgba(37,99,235,0.4), inset 0 1px 0 rgba(255,255,255,0.4)",
							}}
						>
							<span className="text-white text-xs font-black tracking-tight">AT</span>
						</div>
						<span
					...
					<div className="flex items-baseline gap-1">
						...
						<span className="text-sm font-semibold text-slate-600/60">{ampm}</span>
					</div>
				</header>
					...
					<div className="w-full max-w-2xl">
						<div className="grid grid-cols-4 gap-3">
							{tiles.map((tile, i) => {
								...
								if (tile.kind === "brew") {
									return <ChannelTile key="brew" kind="brew" onClick={() => navigate("/store")} />;
								}
								return (
									<ChannelTile
										key={tile.channel.id}
				...
				<footer className="relative flex items-center justify-between px-6 pb-4 pt-2 shrink-0">
					...
					<button
						...
						style={{
							...
							boxShadow: "0 2px 8px rgba(100,160,240,0.3), inset 0 1px 0 rgba(255,255,255,0.6)",
						}}
						aria-label="Home"
					>
					...
					<button
						...
						aria-label="ログアウト"
					>
						<svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5 text-slate-600/65">
