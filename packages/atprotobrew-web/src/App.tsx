...
export default function App() {
	...
	const [installedChannels, setInstalledChannels] = useState<InstalledChannel[]>(loadInstalledFromStorage);
		...
		const fetchLauncherData = async () => {
			...
			try {
				...
				if (launcherData?.view) {
					...
					const channels: InstalledChannel[] = launcherData.view.map((item: any, index: number) => ({
						id: `channel-${index}`,
	...
	return (
		<BrowserRouter>
			<Routes>
				<Route
					path="/"
					element={<Launcher installedChannels={installedChannels} onLogout={logout} isLoading={isLoading} />}
				/>
				<Route
					path="/store"
					element={<Store installedChannels={installedChannels} onInstall={handleInstall} />}
				/>
				<Route
					path="/store/:id"
					element={
